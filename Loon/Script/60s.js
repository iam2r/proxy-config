    ...
    await $.http.get({
        ...
        .then(async ({ body }) => {
            ...
            const getCloseRemark = async () => {
                const data = JSON.parse((await $.http.get(`https://dict.youdao.com/infoline?mode=publish&date=${today}&update=auto&apiversion=5.0`)).body);
            ...
            if ($.isLoon) {
                $notification.post(summary, "", _content, { "media-url": thumb });
              } else {
                $.msg(summary, '', _content, { "media-url": thumb })
              }
