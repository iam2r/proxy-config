  ...
  return new Promise(async (resolve) => {
    ...
    if (_almanac) {
      ...
      _almanac["festivals"]?.length &&
        (content +=
          "\n[节]" +
          _almanac["festivals"].map((item) => `【${item}】`).join(""));
      content +=
        "\n[宜]" +
        _almanac["suit"]
          .map((item) =>
  ...
  return new Promise(async (resolve, reject) => {
    ...
    if (!dataArr.length) {
      ...
      try {
        ...
        const htmlArr = html
          .match(
            /<div class="wnrl_k_you".*>([\s\S]*?)<\/div><div class="wnrl_k_xia_id"/
          )[0]
          ...
          .map((item) => {
            const [, date = ""] = item.match(
              /<div class="wnrl_k_you_id_biaoti">([\s\S]*?)<\/div>/
            ) || ["", ""];
            const [, day = ""] = item.match(
              /<div class="wnrl_k_you_id_wnrl_riqi">([\s\S]*?)<\/div>/
            ) || ["", ""];
...
function getWeekend() {
  ...
  const saturday = new Date(
    now.getFullYear(),
    now.getMonth(),
    now.getDate() + daysUntilWeekend + 1,
    0,
    0,
    0
  );
...
function getRemainDays(timestamp) {
  return Math.floor(
    (timestamp - today.getTime().toString().slice(0, 10)) / 60 / 60 / 24
  );
}
/**
  ...
  calendar = {
    ...
    lYearDays: function (b) {
      var f,
        c = 348;
      for (f = 32768; f > 8; f >>= 1)
        c += calendar.lunarInfo[b - 1900] & f ? 1 : 0;
    ...
    leapDays: function (b) {
      return calendar.leapMonth(b)
        ? 65536 & calendar.lunarInfo[b - 1900]
          ? 30
          : 29
        : 0;
    },
    monthDays: function (b, f) {
      return f > 12 || f < 1
        ? -1
        : calendar.lunarInfo[b - 1900] & (65536 >> f)
        ? 30
        : 29;
    },
    ...
    toGanZhiYear: function (b) {
      ...
      return (
        0 == f && (f = 10),
        0 == c && (c = 12),
        calendar.Gan[f - 1] + calendar.Zhi[c - 1]
      );
    },
    ...
    solar2lunar: function (b, f, c) {
      ...
      for (a = 1; a < 13 && d > 0; a++)
        o > 0 && a == o + 1 && 0 == i
          ? (--a, (i = !0), (r = calendar.leapDays(u)))
          : (r = calendar.monthDays(u, a)),
          1 == i && a == o + 1 && (i = !1),
          (d -= r);
      0 == d && o > 0 && a == o + 1 && (i ? (i = !1) : ((i = !0), --a)),
        d < 0 && ((d += r), --a);
      var h = a,
        D = d + 1,
        g = f - 1,
      ...
      var M = !1,
        T = null;
      p == c && ((M = !0), (T = calendar.solarTerm[2 * f - 2])),
        m == c && ((M = !0), (T = calendar.solarTerm[2 * f - 1]));
    ...
    lunar2solar: function (b, f, c, e) {
      ...
      var r = calendar.monthDays(b, f),
        d = r;
      if ((e && (d = calendar.leapDays(b, f)), b < 1900 || b > 2100 || c > d))
        return -1;
      ...
      for (t = 1; t < f; t++)
        (s = calendar.leapMonth(b)),
          l || (s <= t && s > 0 && ((n += calendar.leapDays(b)), (l = !0))),
          (n += calendar.monthDays(b, t));
      e && (n += r);
